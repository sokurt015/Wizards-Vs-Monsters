...
public class ShopSlide : MonoBehaviour
{
    public RectTransform currentPanel; // shown at start
    public RectTransform nextPanel;    // offscreen bottom initially
    ...
    private void Start()
    {
        ...
        if (nextPanel != null)
        {
            // Put next panel OFFSCREEN at bottom
    ...
    public void SlideForward()
    {
        ...
        StartCoroutine(SlidePanels(
            from: currentPanel,
            fromEnd: new Vector2(0, screenHeight),        // slide current UP offscreen
            to: nextPanel,
            toStart: new Vector2(0, -screenHeight),       // next panel starts offscreen BOTTOM
            toEnd: Vector2.zero,                          // ends centered
            onComplete: () =>
            {
                currentPanel = nextPanel;       // Now next becomes current
                ResetScrollPosition();    // Reset scroll position to top after slide
            }));
    }
    ...
    public void SlideBackward()
    {
        ...
        StartCoroutine(SlidePanels(
            from: currentPanel,
            fromEnd: new Vector2(0, -screenHeight),       // current slides DOWN offscreen
            to: previousPanel,
            toStart: new Vector2(0, screenHeight),        // previous slides in from TOP
            toEnd: Vector2.zero,
            onComplete: () =>
            {
    ...
    {
        // Set the verticalNormalizedPosition to 0 to scroll to the top
        if (scrollRect != null)
        {
